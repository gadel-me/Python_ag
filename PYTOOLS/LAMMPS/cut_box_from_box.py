...
def cutting(main_sys, cutting_box, inverse=True, frame_id=-1, shift=None):
    ...
    face_4 = [
        ...
        for i in agv.get_plane(
            cutting_box.crt_a, cutting_box.crt_b, vt_p=cutting_box.crt_c + shift
        )
    ]
    face_5 = [
        -1 * i
        for i in agv.get_plane(
            cutting_box.crt_c, cutting_box.crt_a, vt_p=cutting_box.crt_b + shift
        )
    ]
    face_6 = [
        -1 * i
        for i in agv.get_plane(
            cutting_box.crt_b, cutting_box.crt_c, vt_p=cutting_box.crt_a + shift
        )
    ]
    main_sys.cut_shape(
        frame_id, inverse, face_1, face_2, face_3, face_4, face_5, face_6
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-rep", default=None, nargs=6, type=int, metavar=4, help=rep_help
    )
    parser.add_argument(
        "-shft",
        default=None,
    ...
    parser.add_argument(
        "-lmpdat_cutshape", default=None, metavar="*.lmpdat", help=lmpdat_help
    )
    ...
    parser.add_argument(
        "-rep_cutshape", default=None, nargs=6, type=int, metavar=12, help=rep_help
    )
    parser.add_argument(
        "-shft_cutshape",
        default=None,
    ...
    parser.add_argument(
        "-exact_cutshape", default=False, action="store_true", help=cut_by_shape_help
    )
    ...
    boxtype_help = (
        "Box types: 'lammps' | 'cartesian' | 'lattice'\n"
    ...
    parser.add_argument(
        "-inverse", default=False, action="store_true", help=inverse_help
    )
    parser.add_argument(
        "-shft_cut_result",
        nargs=3,
    ...
    # ===============================================================================
    sys_cutfrom = ag_lammps.read_lmpdat(
        args.lmpdat, dcd=args.dcd, frame_idx_start=args.f, frame_idx_stop=args.f
    )

    # replicate cell if desired
    if args.rep is not None:
        # read last frame since the frame selection was done with read_lmpdat already
        sys_cutfrom.replicate_cell(
            n_start=args.rep[0],
            n_stop=args.rep[1],
            direction="a",
            frame_id=-1,
    ...
    if args.lmpdat_cutshape is not None or args.dcd_cutshape is not None:
        sys_cutshape = ag_lammps.read_lmpdat(
            args.lmpdat_cutshape,
            dcd=args.dcd_cutshape,
            frame_idx_start=args.f_cutshape,
            frame_idx_stop=args.f_cutshape,
        )

        # replicate cell if desired
        if args.rep_cutshape is not None:
            sys_cutshape.replicate_cell(
                n_start=args.rep_cutshape[0],
                n_stop=args.rep_cutshape[1],
                direction="a",
                frame_id=-1,
                adjust_box=True,
            )
            sys_cutshape.replicate_cell(
                n_start=args.rep_cutshape[2],
                n_stop=args.rep_cutshape[3],
                direction="b",
                frame_id=-1,
                adjust_box=True,
            )
            sys_cutshape.replicate_cell(
                n_start=args.rep_cutshape[4],
                n_stop=args.rep_cutshape[5],
                direction="c",
                frame_id=-1,
                adjust_box=True,
            )
