...
parser.add_argument(
    "-fst", action="store", type=int, default=0, help="Thermo output from first frame."
)

parser.add_argument(
    "-lst", action="store", type=int, default=-1, help="Thermo output from last frame."
)

group_dor2.add_argument(
    "-linreg",
    dest="linreg",
    action="store_true",
    default=True,
...
parser.add_argument(
    "-fit", "--fit_function", dest="fit", help="use a function to fit plotted data."
)
...
while keep_plotting in ["Yes", "Y", "yes", "y", ""]:
    ...
    while wrong_lst is True:
        ...
        if args.lst > num_frames:
            ...
            print(
                "Last frame ({}) smaller than first frame! ({})".format(
                    args.lst, args.fst
                )
            )
        else:
