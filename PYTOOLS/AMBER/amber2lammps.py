...
parser.add_argument(
    "prmtop", metavar="*.prmtop", action="store", help="Amber Topology File",
)

parser.add_argument(
    "-xyz", metavar="*.xyz", action="store", help="XYZ-File with coordinates",
)

parser.add_argument(
    "-inpcrd", metavar="*.inpcrd", action="store", help="Amber-Coordinates file."
)

parser.add_argument("-mdcrd", metavar="*.mdcrd", help="Amber-Trajectory file.")
...
parser.add_argument(
    "-sysname", default="UNK", type=str, help="Name of the molecule/system"
)

parser.add_argument(
    "-out", default="DEFAULTNAME.lmpdat", action="store", help="Name of output-file."
)
...
amber_topology.ui_convert_units(
    energy_unit_out="eV", ang_unit_out="deg", cvff_style=False
)
...
amber_topology.write_lmpdat(
    args.out, frame_id=0, title="{}".format(args.sysname), cgcmm=True
)
