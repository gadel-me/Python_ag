...
parser.add_argument("-frcmod", help="First frcmod file from antechamber output")
...
for prm_idx in range(num_params):
    ...
    if os.path.isfile(set_params_out) is False:
        set_parms = subprocess.run([paramfit, "-i", set_params_in, "-p", cur_prmtop])
        time.sleep(2)
    ...
    if os.path.isfile(fit_k_out) is False:
        with open(fit_k_out, "w") as out:
            ...
            if fit_K != 0:
                raise Warning(
                    "Something went wrong with paramfit during 'derivation of K'."
                )
    ...
    if os.path.isfile(fit_params_out) is False:
        with open(fit_params_out, "w") as out:
            ...
            if fit_param != 0:
                raise Warning(
                    "Something went wrong with paramfit during 'fitting parameters'!"
                )
