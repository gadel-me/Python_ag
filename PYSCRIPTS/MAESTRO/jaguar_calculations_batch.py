...
parser.add_argument("-jag_folders", nargs="*", action="store", required=True)
...
for jag_folder in args.jag_folders:
    ...
    jaguar_run = subprocess.run(
        ["{}/jaguar".format(schrodinger), "run", "-TPP", "4", jaguar_in]
    )
    os.chdir(workdir)
    print("Returning to ", workdir)
    print(
        "Waiting for {} h (hopefully job finishes earlier)!".format(
            args.estimated_duration / 3600
        )
    )
