...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=size,
        help="Number of cores for quenching subroutine",
    )
    ...
    for curcycle in remaining_cycles:
        ...
        lmpsettings_quench = aglmpsim.LmpSim(
            tstart=args.quench_tstart,
            tstop=args.quench_tstop,
            logsteps=args.quench_logsteps,
            runsteps=args.quench_steps,
            pc_file=args.pair_coeffs,
            settings_file=args.set,
            input_lmpdat=sysprep_out_lmpdat,
            inter_lmprst=quench_rst,
            output_lmprst=quench_out,
            output_dcd=quench_dcd,
            output_lmplog=quench_log,
            gpu=args.gpu,
            ncores=args.quench_np,
        ...
        lmpsettings_relax_cut = aglmpsim.LmpSim(
            tstart=args.relax_cut_tstart,
            tstop=args.relax_cut_tstop,
            pstart=args.relax_cut_pstart,
            pstop=args.relax_cut_pstop,
            logsteps=args.relax_cut_logsteps,
            runsteps=args.relax_cut_steps,
            pc_file=args.solvent_paircoeffs,
            settings_file=args.settings_solvent,
            input_lmpdat=cut_solv_lmpdat,
            inter_lmprst=cut_solv_rst,
            output_lmprst=cut_solv_out,
            output_dcd=cut_solv_dcd,
            output_lmplog=cut_solv_log,
            gpu=args.solvent_gpu,
            dielectric=args.dielectric,
        )
        ...
        lmpsettings_void = aglmpsim.LmpSim(
            ...
            pstart=args.void_pstart,
            pstop=args.void_pstop,
            logsteps=args.void_logsteps,
            runsteps=args.void_steps,
            pc_file=args.solvent_paircoeffs,
            settings_file=args.settings_solvent,
            input_lmpdat=lmpsettings_relax_cut.input_lmpdat,
            input_lmprst=lmpsettings_relax_cut.output_lmprst,
            inter_lmprst=void_solv_rst,
            output_lmprst=void_solv_out,
            output_dcd=void_solv_dcd,
            output_lmplog=void_solv_log,
            gpu=args.solvent_gpu,
            dielectric=args.dielectric,
        )
        ...
        if args.lmps is not None:
            solution_lmpdat = (
                anneal_dir + "solution_{}".format(curcycle) + "_out.lmpdat"
            )
        else:
        ...
        lmpsettings_relax_solv = aglmpsim.LmpSim(
            ...
            pstart=args.relax_solv_pstart,
            pstop=args.relax_solv_pstop,
            logsteps=args.relax_solv_logsteps,
            runsteps=args.relax_solv_steps,
            pc_file=args.solution_paircoeffs,
            settings_file=args.set,
            input_lmpdat=solution_lmpdat,
            inter_lmprst=relax_solv_rst,
            output_lmprst=relax_solv_out,
            output_dcd=relax_solv_dcd,
            output_lmplog=relax_solv_log,
            gpu=args.gpu,
            dielectric=args.dielectric,
        )
        ...
        lmpsettings_heat = aglmpsim.LmpSim(
            tstart=args.heat_tstart,
            tstop=args.heat_tstop,
            pstart=args.heat_pstart,
            pstop=args.heat_pstop,
            logsteps=args.heat_logsteps,
            runsteps=args.heat_steps,
            pc_file=args.solution_paircoeffs,
            settings_file=args.set,
            input_lmpdat=solution_lmpdat,
            input_lmprst=None,
            inter_lmprst=heat_rst,
            output_lmprst=heat_out,
            output_dcd=heat_dcd,
            output_lmplog=heat_log,
            gpu=args.gpu,
            dielectric=args.dielectric,
        )
        ...
        lmpsettings_anneal = aglmpsim.LmpSim(
            tstart=args.anneal_tstart,
            tstop=args.anneal_tstop,
            pstart=args.anneal_pstart,
            pstop=args.anneal_pstop,
            logsteps=args.anneal_logsteps,
            runsteps=args.anneal_steps,
            pc_file=args.solution_paircoeffs,
            settings_file=args.set,
            input_lmpdat=solution_lmpdat,
            input_lmprst=lmpsettings_heat.output_lmprst,
            inter_lmprst=anneal_rst,
            output_lmprst=anneal_out,
            output_dcd=anneal_dcd,
            output_lmplog=anneal_log,
            gpu=args.gpu,
            dielectric=args.dielectric,
        )
        ...
        lmpsettings_requench = aglmpsim.LmpSim(
            ...
            logsteps=args.requench_logsteps,
            runsteps=args.requench_steps,
            pc_file=args.pair_coeffs,
            settings_file=args.set,
            input_lmpdat=requench_lmpdat,
            inter_lmprst=requench_rst,
            output_lmprst=requench_out,
            output_dcd=requench_dcd,
            output_lmplog=requench_log,
            dielectric=args.dielectric,
        )
        ...
        while not os.path.isfile(lmpsettings_requench.output_lmprst):
            ...
            while not os.path.isfile(lmpsettings_anneal.output_lmprst):
                ...
                if os.path.isfile(lmpsettings_anneal.output_lmprst) is False:
                    if rank == 0:
                        ...
                        solvate_sys.def_boxes_by_coords(
                            addition=(20, 20, 20), boxtype="lammps"
                        )
                    ...
                    if args.lmps is not None:
                        ...
                        if not os.path.isfile(lmpsettings_void.output_lmprst):
                            for _ in range(5):
                                no_clashes = agk.create_voids(
                                    lmpsettings_void,
                                    lmpsettings_quench.input_lmpdat,
                                    lmpsettings_quench.output_dcd,
                                    lmpsettings_relax_cut.output_dcd,
                                )
                                ...
                                os.rename(
                                    lmpsettings_void.output_lmprst,
                                    lmpsettings_void.inter_lmprst,
                                )
                                exit(102)
                        ...
                        if rank == 0 and not os.path.isfile(solution_lmpdat):
                            aglmp.write_lmpdat(
                                solution_lmpdat,
                                lmpsettings_sysprep.output_lmpdat,
                                lmpdat_b=lmpsettings_relax_cut.input_lmpdat,
                                dcd_a=lmpsettings_quench.output_dcd,
                                dcd_b=lmpsettings_void.output_dcd,
                                box_from_b=True,
                            )
                    ...
                    if rank == 0:
                        ...
                        aggregate_ok = solution_sys.check_aggregate(
                            -1,
                            excluded_atm_idxs=solution_sys_atoms_idxs[
                                solvate_sys_natoms:
                            ],
                        )
            ...
            if rank == 0:
                ...
                if os.path.isfile(lmpsettings_requench.input_lmpdat) is False:
                    ...
                    anneal_lmplog_files = glob.glob(
                        r"{}/*[0-9]_anneal_[0-9]*.lmplog".format(anneal_dir)
                    )
                    anneal_dcds = glob.glob(
                        r"{}/*[0-9]_anneal_[0-9]*.dcd".format(anneal_dir)
                    )
                    # find frame which scores best (dcd and idx) and its value
                    best_dcd, best_idx, best_val = agk.find_best_frame(
                        anneal_lmplog_files,
                        anneal_dcds,
                        thermo="c_pe_solvate_complete",
                        percentage_to_check=percentage_to_check,
                    )
                    agk.write_to_log(
                        "{}, {}, {} (eV)".format(best_dcd, best_idx, best_val)
                    )
                    agk.write_requench_data(
                        lmpsettings_sysprep.output_lmpdat,
                        best_dcd,
                        best_idx,
