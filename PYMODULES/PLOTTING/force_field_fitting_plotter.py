...
def plot_all(
    ref_file, data_files, xlabel=None, title=None, substract=False, x_offset=None
):
    """
    Plot all data.
    ...
    def plot_data_files():
        ...
        for index, data_file in enumerate(data_files):
            ...
            label = r"{:> 6} {} - $\chi^2$-err:{:> 8.7f} eV".format(
                iteration, addname, chi_square_error
            )
            color = cmap(norm(index))
            if x_offset is not None:
                plt.plot(
                    [i + x_offset if i < 0 else i for i in data_x],
                    data_y,
                    linestyle=lstyle,
                    marker=marker,
                    linewidth=0.2,
                ...
                plt.plot(
                    [i + x_offset if i < 0 else i for i in ref_x],
                    interp_y,
                    linestyle=lstyle,
                    marker=marker,
                    linewidth=0.2,
            ...
            if substract is True:
                ...
                if x_offset is not None:
                    plt.plot(
                        ...
                        marker=marker,
                        color=red,
                        label=r"{:> 6} vs. ab initio: $\Delta$ {}".format(
                            iteration, addname
                        ),
                    )
                else:
...
def plot_multiple_histo(
    ...
    **data : dict {str : list of floats}
    ...
    for idx, (key, data) in enumerate(data_dict.items()):
        ...
        # measured histogram
        nhist, bins, patches = axes[idx].hist(
            data,
            bins=x_range,
            density=True,
            histtype="step",
            align="mid",
            color=pantone_colors_hex["blue"],
        )
        ...
        axes[idx].text(
            0.02,
            0.97,
            "Form {} $\mu={:.3f}$ $\sigma={:.3f}$ $VarCoeff={:.3f}$ %".format(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "ab_result_file", help="Two column file with ab initio entities and values"
    )
    parser.add_argument(
        "md_result_files",
        nargs="*",
    ...
    parser.add_argument("-xlabel", default=None, help="Title of x-axis")
